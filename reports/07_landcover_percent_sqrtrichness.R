...
allInsects.long <- allInsects %>% 
  select(richness.t, Urban_1000, Agriculture_500, Open.uncultivated.land_50, Wetland_50, Forest_1000) %>% pivot_longer(-c(richness.t), names_to = "landcover", values_to = "cover")
...
meancover$landcover_names <- plyr::mapvalues(meancover$landcover, from = c("Urban_1000", "Agriculture_500", "Open.uncultivated.land_50", "Wetland_50", "Forest_1000"), to = c("Urban", "Farmland", "Grassland", "Wetland", "Forest"))
...
model2 <- lme(richness ~ Agriculture_500 + 
                Forest_1000 +
                Wetland_50 +
                Urban_1000 +
...
tab_model(model2, dv.labels = c("Richness"), pred.labels = c("Intercept", "Farmland (500 m)", "Forest (1000 m)", "Wetland (50 m)", "Urban (1000 m)", "Grassland (50 m)", "Time band: midday vs evening", "Temperature (20-25 degree celsius)", "Temperature (25-30 degree celsius)", "Wind (light breeze)", "Wind (moderate breeze)", "Sampling time", "Potential stops", "Day of year", "Time within evening
...
temp <- effectdata$Open.uncultivated.land_50
temp$landcover <- "Grassland_50"
grass <- temp %>% 
  dplyr::rename(
    propcover = Open.uncultivated.land_50
  ) %>% select(landcover, propcover, fit, se, lower, upper)
...
cowplot::save_plot("plots/effect_plot_richness_t.png", effectplot_r, base_width = 12, base_height = 8)
...
cowplot::save_plot("plots/effect_plot_time_notimeband_richness_t.png", effectplot_time_r, base_width = 14, base_height = 7)
...
tab_model(model_midday_r, model_evening_r, dv.labels = c("Richness (midday)", "Richness (evening)"), pred.labels = c("Intercept", "Farmland", "Forest", "Wetland", "Urban", "Grassland", "Sampling time", "Temperature (20-25)", "Temperature (25-30)", "Wind (light breeze)", "Wind (moderate breeze)", "Sampling duration", "Potential stops", "Day of year"),
          string.pred = "Coefficient",
          string.ci = "Conf. Int (95%)",
          string.p = "P-Value")
