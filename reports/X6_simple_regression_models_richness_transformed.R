...
# best normalisation - only need to be transformed on size sorted samples
library(bestNormalize)

testobj <- allInsects_totsample$richness
MASS::truehist(testobj, nbins = 12)

# Perform some tranformations individually

# arcsinh transformation
(arcsinh_obj <- arcsinh_x(testobj))
# Box Cox's Transformation
(boxcox_obj <- boxcox(testobj))
# Yeo-Johnson's Transformation
(yeojohnson_obj <- yeojohnson(testobj))
# orderNorm Transformation
(orderNorm_obj <- orderNorm(testobj))
# Pick the best one automatically
(BNobject <- bestNormalize(testobj))

xx <- seq(min(testobj), max(testobj), length = 100)
...
allInsects$richness.t <- BNobject$x.t # for size sorted data
allInsects_totsample$richness.t <- sqrt(allInsects_totsample$richness)# for total samples

# urban
fit = lm((allInsects_totsample$richness.t) ~ allInsects_totsample$Urban_1000)
coef(summary(fit))[2,4] 

qU <- ggplot(allInsects_totsample,aes(x=(Urban_1000),y=(richness.t)))+
...
fit = lm((allInsects_totsample$richness.t) ~ (allInsects_totsample$Agriculture_1000))
coef(summary(fit))[2,4] 

qF <- ggplot(allInsects_totsample,aes(x=Agriculture_1000,y=(richness.t)))+
...
fit = lm((allInsects_totsample$richness.t) ~ allInsects_totsample$Open.uncultivated.land_1000)
coef(summary(fit))[2,4] 

qD <- allInsects_totsample %>% filter(Open.uncultivated.land_1000 < 0.4) %>% ggplot(aes(x=Open.uncultivated.land_1000, y=(richness.t)))+
...
fit = lm((allInsects_totsample$richness.t) ~ allInsects_totsample$Heathland_1000)
coef(summary(fit))[2,4] 

qH <- allInsects_totsample  %>% ggplot(aes(x=Heathland_1000, y=(richness.t)))+
...
fit = lm((allInsects_totsample$richness.t) ~ allInsects_totsample$Wetland_1000)
coef(summary(fit))[2,4] 

qW <- ggplot(allInsects_totsample,aes(x=Wetland_1000,y=(richness.t)))+
...
fit = lm((allInsects_totsample$richness.t) ~ allInsects_totsample$Forest_1000)
coef(summary(fit))[2,4] 

qFo <- ggplot(allInsects_totsample,aes(x=Forest_1000,y=(richness.t)))+
...
#qUns <- ggplot(allInsects_totsample,aes(x=(Unspecified.land.cover_1000),y=(Biomass+1)))+geom_point(col="darkgrey")+scale_y_log10() +theme_bw() +geom_smooth(method="lm",color="grey70")+xlab("Unspecified land cover") +ylab("") + labs(subtitle = "F")+ scale_x_continuous(labels = function(x) paste0(x*100, "%"))

y.grob <- textGrob("ASV richness (sqrt-transformed)", 
...
save_plot("plots/DK_Landcover_percent_asvrichness.t.png", plot, base_width = 3, base_height = 8)

### DK plot buffers#################################################

### urban ####
mini_data <- allInsects_totsample %>% select(PCRID,  Urban_50, Urban_250, Urban_500, Urban_1000, richness.t)

# wide to long format
dt_long <- mini_data %>% 
  tidyr::gather(landcover_type, propcover, -PCRID, -richness.t)
#dt_long
...
u <- dt_long %>% mutate(
  ...
  geom_point(aes(x=propcover,y=(richness.t)), show.legend = F, size = 3, colour = landuseCols[1]) +
  facet_wrap(. ~ landcover_type, labeller = landcover_labeller, ncol = 2) +
  theme_pubclean() + xlab("Urban land cover") + ylab("") + scale_x_continuous(
    limits = c(0, 1),
    labels = function(x)
      paste0(x * 100, "%")) 
...
upubr <- dt_long %>% mutate(
  landcover_type = fct_relevel(
    ...
    "Urban_500",
    "Urban_1000"
  )) %>% ggscatter(x = "propcover", y = "richness.t",
                   ...
                   cor.coeff.args = list(method = "spearman")
                                 ...
                                 if_else(readr::parse_number(..p.label..) < 0.001, 
                                         "p<0.001", ..p.label..), sep = "~`,   `~")), label.x = 0.5, label.y = 20) + xlab("Urban land cover") + ylab("") + scale_x_continuous(     limits = c(0, 1),     labels = function(x)       paste0(x * 100, "%"))  + facet_wrap(. ~ landcover_type, labeller = landcover_labeller, ncol = 2) + theme_pubclean()

### farmland ############
mini_data <- allInsects_totsample %>% select(PCRID,  Agriculture_50, Agriculture_250, Agriculture_500, Agriculture_1000, richness.t)
...
a <- dt_long %>% mutate(
  ...
  geom_point(aes(x=propcover,y=richness.t), show.legend = F, size = 3, colour = landuseCols[2]) +
  facet_wrap(. ~ landcover_type, labeller = landcover_labeller, ncol = 2) +
  theme_pubclean() + xlab("Farmland cover") + ylab("") + scale_x_continuous(limits = c(0, 1),     labels = function(x)       paste0(x * 100, "%")) 
  ...
  landcover_type = fct_relevel(
    ...
    "Agriculture_1000"
                   ...
                   cor.coeff.args = list(method = "spearman")
                                 ...
                                 if_else(readr::parse_number(..p.label..) < 0.001, 
                                         "p<0.001", ..p.label..), sep = "~`,   `~")), label.x = 0.5, label.y = 20) + xlab("Farmland cover") + ylab("") + scale_x_continuous(     limits = c(0, 1),     labels = function(x)       paste0(x * 100, "%"))  + facet_wrap(. ~ landcover_type, labeller = landcover_labeller, ncol = 2) + theme_pubclean()

### grassland ###############
mini_data <- allInsects_totsample %>% select(PCRID,  Open.uncultivated.land_50, Open.uncultivated.land_250, Open.uncultivated.land_500, Open.uncultivated.land_1000, richness.t)
...
g <- dt_long %>% mutate(
  ...
  geom_point(aes(x=propcover,y=richness.t), show.legend = F, size = 3, colour = landuseCols[3]) +
  facet_wrap(. ~ landcover_type, labeller = landcover_labeller, ncol = 2) +
  theme_pubclean() + xlab("Grassland cover") + ylab("") + scale_x_continuous(limits = c(0, 1),     labels = function(x)       paste0(x * 100, "%")) 
  ...
  landcover_type = fct_relevel(
    ...
    "Open.uncultivated.land_1000"
                   ...
                   cor.coeff.args = list(method = "spearman")
                                 ...
                                 if_else(readr::parse_number(..p.label..) < 0.001, 
                                         "p<0.001", ..p.label..), sep = "~`,   `~")), label.x = 0.5, label.y = 20) + xlab("Grassland cover") + ylab("") + scale_x_continuous(     limits = c(0, 1),     labels = function(x)       paste0(x * 100, "%"))  + facet_wrap(. ~ landcover_type, labeller = landcover_labeller, ncol = 2) + theme_pubclean()

### heathland ################
mini_data <- allInsects_totsample %>% select(PCRID,  Heathland_50, Heathland_250, Heathland_500, Heathland_1000, richness.t)
...
h <- dt_long %>% mutate(
  ...
  geom_point(aes(x=propcover,y=richness.t), show.legend = F, size = 3, colour = landuseCols[4]) +
  facet_wrap(. ~ landcover_type, labeller = landcover_labeller, ncol = 2) +
  theme_pubclean() + xlab("Heathland cover") + ylab("")+ scale_x_continuous(     limits = c(0, 1),     labels = function(x)       paste0(x * 100, "%")) 
  ...
  landcover_type = fct_relevel(
    ...
    "Heathland_1000"
                   ...
                   cor.coeff.args = list(method = "spearman")
                                 ...
                                 if_else(readr::parse_number(..p.label..) < 0.001, 
                                         "p<0.001", ..p.label..), sep = "~`,   `~")), label.x = 0.5, label.y = 20) + xlab("Heathland cover") + ylab("") + scale_x_continuous(     limits = c(0, 1),     labels = function(x)       paste0(x * 100, "%"))  + facet_wrap(. ~ landcover_type, labeller = landcover_labeller, ncol = 2) + theme_pubclean()

### wetland #############
mini_data <- allInsects_totsample %>% select(PCRID,  Wetland_50, Wetland_250, Wetland_500, Wetland_1000, richness.t)
...
w <- dt_long %>% mutate(
  ...
  geom_point(aes(x=propcover,y=richness.t), show.legend = F, size = 3, colour = landuseCols[5]) +
  facet_wrap(. ~ landcover_type, labeller = landcover_labeller, ncol = 2) +
  theme_pubclean() + xlab("Wetland cover") + ylab("") + scale_x_continuous(     limits = c(0, 1),     labels = function(x)       paste0(x * 100, "%")) 
  ...
  landcover_type = fct_relevel(
    ...
    "Wetland_1000"
                   ...
                   cor.coeff.args = list(method = "spearman")
                                 ...
                                 if_else(readr::parse_number(..p.label..) < 0.001, 
                                         "p<0.001", ..p.label..), sep = "~`,   `~")), label.x = 0.5, label.y = 20) + xlab("Wetland cover") + ylab("") + scale_x_continuous(     limits = c(0, 1),     labels = function(x)       paste0(x * 100, "%"))  + facet_wrap(. ~ landcover_type, labeller = landcover_labeller, ncol = 2) + theme_pubclean()

### forest ###########################
mini_data <- allInsects_totsample %>% select(PCRID,  Forest_50, Forest_250, Forest_500, Forest_1000, richness.t)
...
f <- dt_long %>% mutate(
  ...
  geom_point(aes(x=propcover,y=richness.t), show.legend = F, size = 3, colour = landuseCols[6]) +
  facet_wrap(. ~ landcover_type, labeller = landcover_labeller, ncol = 2) +
  theme_pubclean() + xlab("Forest cover") + ylab("") + scale_x_continuous(     limits = c(0, 1),     labels = function(x)       paste0(x * 100, "%")) 
  ...
  landcover_type = fct_relevel(
    ...
    "Forest_1000"
                   ...
                   cor.coeff.args = list(method = "spearman")
                                 ...
                                 if_else(readr::parse_number(..p.label..) < 0.001, 
                                         "p<0.001", ..p.label..), sep = "~`,   `~")), label.x = 0.5, label.y = 20) + xlab("Forest cover") + ylab("") + scale_x_continuous(     limits = c(0, 1),     labels = function(x)       paste0(x * 100, "%"))  + facet_wrap(. ~ landcover_type, labeller = landcover_labeller, ncol = 2) + theme_pubclean()
...
y.grob <- textGrob("Variation in ASV richness (sqrt normalization)", 
...
ggsave('plots/Landcover_percent_buffer_noreg_richness.t.png', plot = test, width=50,height=35, units = "cm", dpi=300)
...
grid.draw(test)
#ggdraw(add_sub(plot, "Label", vpadding=grid::unit(0,"lines"),y=6, x=0.5, vjust=4.5))
ggsave('plots/Landcover_percent_with_stats_richness.t.png', plot = test, width=50,height=35, units = "cm", dpi=300)
#ggsave("plots/Landcover_percent_with_stats.png",width=6,height=20)

### intensive vs organic farming predicted richness.t ###############################
# Intensive
lme1000 <- lmer(richness.t ~ 
                  Intensiv_1000 +
                  Ekstensiv_1000 +
                  Semi.intensiv_1000 +
                  #Markblok_1000 +
...
colnames(Intensiv)
names(Intensiv)[1] <- "predrichness.t"
...
colnames(propOeko)
names(propOeko)[1] <- "predrichness.t"
...
gls1 <- lme(richness.t ~ Intensiv_1000 +
              Ekstensiv_1000 +
              Semi.intensiv_1000 +
              #Markblok_1000 + 
              Intensiv_organic_1000 +
  ...
  scale_color_manual(
    ...
    labels = function(x)
      paste0(x * 100, "%")) + geom_ribbon(
        ...
        x = "Land cover",
        y = "Predicted effect change in ASV richness",
        subtitle = "A",
...
hist((allInsects_totsample$Agriculture_500)) # does not help
...
hist((allInsects_totsample$Urban_1000)) #sqrt is bettwe
...
lme50 <- lmer(richness.t ~ (Heathland_50) + Time_band + 
                Time_band:cnumberTime + cStops + cyDay + 
                (1|RouteID_JB) + (1|PilotID), data=allInsects_totsample)
lme250 <- lmer(richness.t ~ (Heathland_250) + Time_band + 
                 Time_band:cnumberTime + cStops + cyDay +  
                 (1|RouteID_JB) + (1|PilotID), data=allInsects_totsample)
lme500 <- lmer(richness.t ~ (Heathland_500) + Time_band + 
                 Time_band:cnumberTime + cStops + cyDay +  
                 (1|RouteID_JB) + (1|PilotID), data=allInsects_totsample)
lme1000 <- lmer(richness.t ~ (Heathland_1000) + Time_band + 
                  Time_band:cnumberTime + cStops + cyDay +  
                  (1|RouteID_JB) + (1|PilotID), data=allInsects_totsample)
...
ggplot(outAll)+
  ...
  xlab("Buffer size (m)") + ylab("Effect of land cover on ASV richness")+ theme(plot.subtitle=element_text(size=18, face="bold", color="black")) + labs(subtitle = "B")

ggsave("plots/DK_Landcover_buffer_richness.t.png",width=5,height=12)

### max land cover (1000 m) ###########
mini_data <- allInsects_totsample %>% select(PCRID, maxLand_use, maxareaProp, richness.t)
...
hist(mini_data$richness.t)
qqnorm(mini_data$richness.t)
...
) %>% ggplot() + geom_point(aes(x=maxareaProp,y=richness.t, colour = maxLand_use), show.legend = F, size = 3) + facet_wrap(. ~ maxLand_use, nrow = 3) +
...
) %>% ggscatter(x = "maxareaProp", y = "richness.t",
                color = "maxLand_use",
